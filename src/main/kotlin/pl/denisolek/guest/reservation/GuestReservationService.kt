...
class GuestReservationService(
    ...
    fun addReservation(dto: CreateReservationGuestDTO): Int {
        val restaurant = restaurantService.findById(dto.restaurantId) ?: throw ServiceException(
    ...
    private fun prepareReservationCustomer(dto: CreateReservationGuestDTO): Customer {
        return customerService.findOrCreate(
            ReservationCustomerGuestDTO.createCustomer(
    ...
    fun submitCode(reservation: Reservation, code: String) {
        when {
            ...
            reservation.verificationCode != code -> throw ServiceException(
