...
data class SchemaDTO(
    ...
    constructor(restaurant: Restaurant) : this(
            ...
            items = restaurant.floors.flatMap { it.schemaItems.filter { it.type == SchemaItem.Type.ITEM }.map { TypeItemDTO(it) } }.toMutableList(),
            wallItems = restaurant.floors.flatMap { it.schemaItems.filter { it.type == SchemaItem.Type.WALL_ITEM }.map { TypeWallItemDTO(it) } }.toMutableList()
    )
